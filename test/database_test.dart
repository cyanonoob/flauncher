...
void main() {
  ...
  test("listApplications", () async {
    await database.customInsert("INSERT INTO apps(package_name, name, version)"
        " VALUES('com.geert.flauncher', 'FLauncher', '1.0.0');");

    final apps = await database.listApplications();

    expect(apps.length, 1);
    expect(apps[0].packageName, "com.geert.flauncher");
  ...
  test("persistApps", () async {
    ...
    expect(app.read<String>("package_name"), "com.geert.flauncher");
  ...
  test("updateApp", () async {
    await database
        .customInsert("INSERT INTO apps(package_name, name, version, hidden)"
            " VALUES('com.geert.flauncher', 'FLauncher', '1.0.0', false);");
    await database.updateApp(
        "com.geert.flauncher", AppsCompanion(hidden: Value(true)));
  ...
  test("deleteApps", () async {
    await database.customInsert("INSERT INTO apps(package_name, name, version)"
        " VALUES('com.geert.flauncher', 'FLauncher', '1.0.0');");
    await database.customInsert("INSERT INTO apps(package_name, name, version)"
        " VALUES('com.geert.flauncher.2', 'FLauncher 2', '1.0.0');");

    await database.deleteApps(["com.geert.flauncher"]);

    final app = await database.customSelect("SELECT * FROM apps;").getSingle();
    expect(app.read<String>("package_name"), "com.geert.flauncher.2");
  });
  ...
  test("deleteCategory", () async {
    ...
    await database.customInsert(
        ...
        " VALUES($categoryId, 'com.geert.flauncher', 0);");
  ...
  test("deleteAppCategory", () async {
    ...
    await database.deleteAppCategory(categoryId, "com.geert.flauncher");
  ...
  test("insertAppsCategories", () async {
    ...
    expect(appCategory.read<String>("app_package_name"), "com.geert.flauncher");
  ...
  test("listCategoriesWithApps", () async {
    ...
    await database.customInsert("INSERT INTO apps(package_name, name, version)"
        " VALUES('com.geert.flauncher.2', 'FLauncher 2', '1.0.0');");
    await database.customInsert("INSERT INTO apps(package_name, name, version)"
        " VALUES('com.geert.flauncher.3', 'FLauncher 3', '1.0.0');");
    final categoryId = await database.customInsert(
        "INSERT INTO categories(name, 'order') VALUES('Test', 2);");
    await database.customInsert(
        "INSERT INTO apps_categories(category_id, app_package_name, 'order')"
        " VALUES($categoryId, 'com.geert.flauncher', 1);");
    await database.customInsert(
        "INSERT INTO apps_categories(category_id, app_package_name, 'order')"
        " VALUES($categoryId, 'com.geert.flauncher.2', 0);");
    await database.customInsert(
        "INSERT INTO apps_categories(category_id, app_package_name, 'order')"
        " VALUES($categoryId, 'com.geert.flauncher.3', 2);");
