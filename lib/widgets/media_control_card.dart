...
// Mock media state - in a real implementation, this would come from Android's MediaSession
class MediaState {
...
class _MediaControlCardState extends State<MediaControlCard>
  ...
  // Mock media state - replace with real media session integration
  ...
  void initState() {
    ...
    _appImageLoadFuture = _loadAppIcon();
  }
                                ...
                                boxShadow: [
                                  BoxShadow(
                                    color: Colors.black.withAlpha(
                                        (_curvedAnimation.value * 51).round()),
                                    blurRadius: 48,
                                    spreadRadius: 2,
                                  ),
                                ],
  ...
  Future<Tuple2<AppImageType, ImageProvider>?> _loadAppIcon() async {
    if (_mediaState.packageName == null) return null;

    try {
      final appsService = Provider.of<AppsService>(context, listen: false);
      Uint8List bytes = await appsService.getAppIcon(_mediaState.packageName!);
            ...
            Expanded(
              ...
              child: Center(
                child: Container(
                  ...
                  child: Icon(
                    _mediaState.isPlaying ? Icons.pause : Icons.play_arrow,
                    color: Colors.white,
                    size: 32,
                  ),
                ),
  ...
  void _togglePlayPause() {
    ...
    // This would send media commands to the Android MediaSession
    setState(() {
      _mediaState = MediaState(
        packageName: _mediaState.packageName,
        appName: _mediaState.appName,
        title: _mediaState.title,
        artist: _mediaState.artist,
        isPlaying: !_mediaState.isPlaying,
        hasMedia: _mediaState.hasMedia,
      );
    });

    // In a real implementation, you would call something like:
    // await _mediaControlChannel.sendPlayPause();
  }

  void _launchMediaApp(BuildContext context) {
    if (_mediaState.packageName != null) {
      final appsService = context.read<AppsService>();
      final app = appsService.applications.firstWhere(
        (app) => app.packageName == _mediaState.packageName,
        orElse: () => App(
          packageName: _mediaState.packageName!,
          name: _mediaState.appName ?? 'Media App',
          version: '1.0',
          hidden: false,
        ),
      );
...
// - Add MediaControlCard to the home screen widget tree
// - Create a separate row/section for media controls
// - Integrate with Android MediaSession API for real media state
// - Add media control methods to FLauncherChannel
